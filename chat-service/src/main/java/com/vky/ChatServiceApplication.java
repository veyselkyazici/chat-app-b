...
public class ChatServiceApplication {
    ...
    // @Bean
    // public CommandLineRunner commandLineRunner(IChatRoomRepository
    // chatRoomRepository, IChatMessageRepository chatMessageRepository,
    // IUserChatSettingsRepository userChatSettingsRepository) throws Exception {
    //
    //
    // PublicKey recipientPublicKey =
    // loadPublicKey("");
    //
    //
    // return args -> {
    // for (int i = 0; i <= 200; i++) {
    ...
    // aesCipher.doFinal(message.getBytes(StandardCharsets.UTF_8));
    //
    ...
    //
    //
    // String friendId = UUID.nameUUIDFromBytes(String.format("User%04d",
    // i).getBytes()).toString();
    //
    // List<String> participants = Arrays.asList(userId, friendId);
    // ChatRoom chatRoom =
    // chatRoomRepository.save(ChatRoom.builder().participantIds(participants).build());
    // if (i == 100) {
    // for (int j = 0; j < 200; j++) {
    ...
    // aesCipherExtra.init(Cipher.ENCRYPT_MODE, aesKeyExtra, new
    // GCMParameterSpec(128, ivExtra));
    // String extraMessage = "Ekstra mesaj " + j;
    // byte[] encryptedMessageExtra =
    // aesCipherExtra.doFinal(extraMessage.getBytes(StandardCharsets.UTF_8));
    //
    // // d. AES anahtarını RSA ile şifrele
    // Cipher rsaCipherExtra =
    // Cipher.getInstance("RSA/ECB/OAEPWithSHA-256AndMGF1Padding");
    ...
    // PSource.PSpecified.DEFAULT
    // );
    // rsaCipherExtra.init(Cipher.ENCRYPT_MODE, recipientPublicKey,
    // oaepParamsExtra);
    // byte[] encryptedAESKeyExtra =
    // rsaCipherExtra.doFinal(aesKeyExtra.getEncoded());
    //
    // // fullDateTime rastgele üret
    // long startEpoch = Instant.now().minus(30, ChronoUnit.DAYS).toEpochMilli(); //
    // son 30 gün
    // long endEpoch = Instant.now().toEpochMilli();
    // long randomEpoch = ThreadLocalRandom.current().nextLong(startEpoch,
    // endEpoch);
    ...
    //
    // System.out.println("Kullanıcı eklendi: " + username + " ChatRoomId > " +
    // userChatSettings.getChatRoomId());
    // }
    //
    // };
    // }
    //
    // public static PublicKey loadPublicKey(String base64PublicKey) throws
    // Exception {
    ...
    // }
    //
    // public static PrivateKey loadPrivateKey(String base64PrivateKey) throws
    // Exception {
